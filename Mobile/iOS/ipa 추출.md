...
def get_usb_iphone():
    try:
        device = frida.get_device_manager().add_remote_device("192.168.1.202:8878")
        return device
    except Exception as e:
        print("Frida 포트 접속 실패:", e)
        sys.exit(1)
        ...
        with zipfile.ZipFile(ipa_path, 'w', zipfile.ZIP_DEFLATED) as ipa_zip:
            for root, dirs, files in os.walk(payload_root):
                for file in files:
                    ...
                    arcname = os.path.relpath(file_path, os.path.dirname(payload_root))
